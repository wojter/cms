...
const ModalNewComment = ({ isOpen, toggleOpen, refetchName }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch("/api/admin/comments/new", {
        method: "POST",
  ...
  return (
    <>
      ...
      <ModalFL show={isOpen} size="xl" onClose={toggleOpen}>
        <ModalFL.Header>New Comment</ModalFL.Header>
        <ModalFL.Body>
          <form
            id="new-comment-form"
        ...
        <ModalFL.Footer className="justify-between">
          ...
          <Button type="submit" form="new-comment-form" color="success">
            Create Comment
          </Button>
        </ModalFL.Footer>
      </ModalFL>
